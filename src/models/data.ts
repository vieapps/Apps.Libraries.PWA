...
export namespace AppData {
	...
	export var Configuration = {
		app: {
			uris: {
				...
				activations: "https://chomuonsach.com/"
			},
			debug: true,
			offline: false,
			mode: "",
			platform: "",
			name: "chomuonsach.com",
			host: "",
			tracking: {
				google: "UA-3060572-8",
	...
	/** Libraries */
	export var Libraries = new Collections.Dictionary<string, AppModels.Library>();
		...
		getKey: (info?: any, prefix?: string) => {
			...
			if (!AppUtility.isObject(sortby, true)) {
				...
				if (prefix == "B") {
					...
					sortby = AppUtility.isNotEmpty(sortby.Title)
			...
			if (prefix == "B") {
				key = "Descending" != sortby 
					? null
					: filterby && filterby.And && filterby.And.Category && AppUtility.isNotEmpty(filterby.And.Category.StartsWith)
						? prefix + "CAT-" + filterby.And.Category.StartsWith
			...
			// libraries
				...
				key = "Ascending" != sortby 
					? null
					: filterby && filterby.And && filterby.And.OwnerID && AppUtility.isNotEmpty(filterby.And.OwnerID.Equals)
						? prefix + "OWN-" + filterby.And.OwnerID.Equals
