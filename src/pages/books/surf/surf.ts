...
export class SurfBooksPage {
	...
	ionViewDidLoad() {
		...
		this.info.filterBy.And.Category.StartsWith = this.navParams.get("Category");
		this.info.filterBy.And.Category.StartsWith = this.info.filterBy.And.Category.StartsWith || "";
		...
		this.info.title = this.info.filterBy.And.Category.StartsWith != ""
			? "Thể loại: " + this.info.filterBy.And.Category.StartsWith
	...
	ionViewDidEnter() {
		...
		if (this.books.length < 1) {
			var request = AppData.buildRequest(this.info.filterBy, undefined, this.info.pagination, r => {
				if (!AppUtility.isNotEmpty(r.FilterBy.And.Category.StartsWith)) {
					r.FilterBy.And.Category.StartsWith = undefined;
				}
	...
	get(onepage: boolean = true, pageNumber?: number) {
		...
		var query = this.info.filtering && AppUtility.isNotEmpty(this.info.filterBy.Query)
			...
			: "";
		var filterByCategory = this.info.filterBy.And.Category.StartsWith != "";
		...
		if (query != "" || filterByCategory || filterByAuthor) {
			books = books.Where(b => {
				return (query != "" ? AppUtility.indexOf(b.ANSITitle, query) > -1 : true)
					&& (filterByCategory ? AppUtility.indexOf(b.Category, this.info.filterBy.And.Category.StartsWith) == 0 : true)
