...
export class StatisticsService {
	...
	async fetchStatusAsync(onNext?: (d: any) => void, onError?: (e: any) => void) {
		if (AppRTU.isReady()) {
			AppRTU.call("libraries", "statistic", "GET", { "object-identity": "status" });
		}
		else {
			try {
				let response = await AppAPI.GetAsync("libraries/statistic/status");
	...
	async fetchCategoriesAsync(onNext?: (d: any) => void, onError?: (e: any) => void) {
		if (AppRTU.isReady()) {
			AppRTU.call("libraries", "statistic", "GET", { "object-identity": "categories" });
		}
		else {
			try {
				let response = await AppAPI.GetAsync("libraries/statistic/categories");
				let data = response.json();
				if (data.Status == "OK") {
	...
	async fetchAuthorsAsync(onCompleted?: (d: any) => void) {
		...
		if (this.Authors.Index < this.Authors.Chars.length) {
			try {
				let path = "libraries/statistic/authors"
