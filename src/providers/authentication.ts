...
export class AuthenticationService {
	...
	isInAppRole(objectName?: string, role?: string, privileges?: Array<AppModels.Privilege>) {
		...
		let privilege = privileges
			? new List(privileges).FirstOrDefault(p => p.ServiceName == "libraries" && p.ObjectName == objectName)
			: undefined;
		return privilege != undefined && privilege.Role == role;
	}

	/** Checks to see the account is has a specific role of the object */
	isInObjectRole(accountID: string, accountRoles: Array<string>, objectUsers: Collections.Set<string>, objectRoles: Collections.Set<string>) {
		return (objectUsers && objectUsers.contains(accountID))
		 || (accountRoles && objectRoles && new List(accountRoles).FirstOrDefault(role => objectRoles.contains(role)) != undefined);
	}
